...
def start(message):
    ...
    # Yahan HTML use kar rahe hain bold ke liye
...
def handle_all(message):
    if message.from_user.id == ADMIN_ID and message.reply_to_message:
        target_id = None
        if message.reply_to_message.forward_from: target_id = message.reply_to_message.forward_from.id
        else:
        ...
        for u in users_col.find():
            try:
                # Text ko process kar rahe hain: '*' replace ho raha hai '<b>' se
                text = (message.text or message.caption or "").replace('*', '<b>', 1).replace('*', '</b>', 1)
                if message.content_type == 'photo': bot.send_photo(u['uid'], message.photo[-1].file_id, caption=text, parse_mode='HTML')
                elif message.content_type == 'video': bot.send_video(u['uid'], message.video.file_id, caption=text, parse_mode='HTML')
                elif message.content_type == 'document': bot.send_document(u['uid'], message.document.file_id, caption=text, parse_mode='HTML')
                else: bot.send_message(u['uid'], text, parse_mode='HTML')
            except: continue
